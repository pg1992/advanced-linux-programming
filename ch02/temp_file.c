...
/* main function for testing the write to and read from temp files. */
int main(int argc, char **argv)
{
    /* Declare a handle. */
    temp_file_handle tf;
    char *input;
    /* Set the input buffer as the first argument or "Hello World" if
     * nothing is passed. */
    if (argc > 1)
        input = argv[1];
    else
        input = "Hello World!";
    /* Write to a temp file and get a handle. */
    tf = write_temp_file(input, strlen(input));
    char *output;
    size_t l;
    /* Read from the temp file pointed by the handle and prints
     * to STDOUT. */
